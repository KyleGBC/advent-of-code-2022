...
impl Board {
    ...
    fn cube_next(&self, p: Pose) -> Pose {
        ...
        let (new_face, new_facing) = match (self.get_face_index(&p), p.f) {
            ...
            (9, F::Left) => (1, F::Down),
            (i, f) => {
                unimplemented!("{i}, {f} not found. This only holds for my specific cube layout")
            }
        };
        ...
        let (x_inset, y_inset) = (
            (self.face_sizes - 1) - x_offset,
            (self.face_sizes - 1) - y_offset,
        );
        ...
        let (x_offset, y_offset) = match (p.f, new_facing) {
            (F::Up, F::Up) | (F::Down, F::Down) | (F::Right, F::Left) | (F::Left, F::Right) => {
                (x_offset, y_inset)
            }
            (F::Up, F::Down) | (F::Down, F::Up) | (F::Left, F::Left) | (F::Right, F::Right) => {
                (x_inset, y_offset)
            }
            (F::Up, F::Right) | (F::Down, F::Left) | (F::Right, F::Up) | (F::Left, F::Down) => {
                (y_offset, x_offset)
            }
            (F::Up, F::Left) | (F::Down, F::Right) | (F::Left, F::Up) | (F::Right, F::Down) => {
                (y_inset, x_inset)
            }
        };
    ...
    fn naive_next(&self, p: Pose, wrapping: bool) -> Option<Pose> {
        ...
        if wrapping {
            (x, y) = (
                x.rem_euclid(self.tiles[0].len() as i32),
                y.rem_euclid(self.tiles.len() as i32),
            );
        }
...
fn main() {
    ...
    let (height, width) = (
        ...
        board_str.lines().next().unwrap().chars().count(),
    );
    ...
    for line in board_str.lines() {
        ...
        line.chars().for_each(|c| tile_line.push(Tile::from_char(c)));
    ...
    let first_open = board.tiles.first().unwrap().iter().position(|t| matches!(t, Tile::Open)).unwrap();
    let mut part1 = Pose { x: first_open, y: 0, f: Facing::Right };
    let mut part2 = part1.clone();

    for ins in instructions {
        let (count, turn) = ins.split_at(ins.find(|c: char| c.is_alphabetic()).unwrap_or(ins.len()));
