...
fn main() {
    ...
    for line in lines.split("\r\n") {
        ...
        let enemy_move = Move::from_string(moves.first().unwrap());
        let my_move = Move::from_string(moves.first().unwrap());
        let outcome = Outcome::calculate(enemy_move, my_move);
        p1_total += my_move.to_points() + outcome.to_points();

        let desired_outcome = Outcome::from_string(moves.first().unwrap());
